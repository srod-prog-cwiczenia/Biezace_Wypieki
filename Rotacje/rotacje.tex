...
\begin{theorem}
  Suppose that $(\mu)_{j=0}^\infty$ and $\theta_0,\ldots, \theta_{k-1}$
...
\]
Moreover, $\Phi_2(E^*)$ is a perfect ``fat'' set.  
...
\[
  E^* \subseteq \bigcup_{j = 0}^\infty \phi_{\mu_j}[M \times \real].
\]
...
Then there exists an open $W^* \subseteq \real^2$ and natural number $n^*$
such that $\emptyset \not= W^* \cap P^* \subseteq \phi_{\mu_{n^*}}[M \times \real]$.
...
Denote:
\begin{align*}
\phi^{-1}_{\mu_{n^*}}(X_0) &= (x(\mu), y(\mu)) \\
\phi^{-1}_{\theta_k}(X_0) &= (x^{(k)}_0, y^{(k)}_0) \qquad \text{for }k = 0,\ldots, K-1 
\end{align*}

Evidently
$x(\mu) \in M$ and $x^{(k)}_0 \in \Phi(E)$ for $k = 0, \ldots, K-1$.
\todo{(...)}
\end{proof}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
%                        The Bibliography                                   %

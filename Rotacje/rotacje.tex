...
Let us note here that this notion is a generalization of a property introduced in
\cite{Laczkovich1998}. Namely, in Lemma 2 of the aforementioned paper \cite{Laczkovich1998},
for the sets $F_1$, $F_2$, we assume that whenever $A_j$ (for $j = 1,2$) is a portion
of $F_j$, then $\operatorname{int}(A_1 + A_2) \not= \emptyset$.
It is easy to see that this is equivalent to the
condition $\frac{\pi}{4} \in \NICEDIRECTIONS(F_1 \times F_2)$.
...
\begin{lemma}\label{lemma:change-in-density}
...
\begin{equation}\label{equation:four-sinus}
...
\item
  ...
  $\mu((x^{(l)}_0 - \eta, x^{(l)}_0 + \eta) \cap \Phi(E)) \geq (1 - \varepsilon) 2 \eta$
  (such an $\eta_0$ exists, since $x^{(l)}_0\in \Phi(E)$ and $\mu(\Phi(E) \triangle E) = 0$).
\end{enumerate}

Suppose that $\hat{x}(\mu)$ is such that
$|\hat{x}(\mu) - x(\mu)| < \frac{\eta_0}{2}$.
Then we have for each $l = 0,\ldots,k-1$:
\[(\hat{x}(\mu) \cos \theta^*_l - y(\mu) | \sin \theta^*_l | - \frac{\eta_0}{2};
(\hat{x}(\mu) \cos \theta^*_l - y(\mu) | \sin \theta^*_l | + \frac{\eta_0}{2})\subseteq\]
\[
(\hat{x}(\mu) \cos \theta^*_l - y(\mu) | \sin \theta^*_l | - \eta_0;
(\hat{x}(\mu) \cos \theta^*_l - y(\mu) | \sin \theta^*_l | + \eta_0).
\]
By Lemma \ref{lemma:change-in-density} we obtain:
\[\mu(
(\hat{x}(\mu) \cos \theta^*_l - y(\mu) | \sin \theta^*_l | - \frac{\eta_0}{2};
(\hat{x}(\mu) \cos \theta^*_l - y(\mu) | \sin \theta^*_l | + \frac{\eta_0}{2})
\cap \Phi(E)) \geq \eta_0 (1 - 2 \varepsilon).
\]
Note that for every $l = 0, \ldots, k-1$ we have
$
\hat{x}(\mu) \cos \theta^*_l - \hat{y}(\mu) \sin \theta^*_l \in
(\hat{x}(\mu) \cos \theta^*_l - y(\mu) | \sin \theta^*_l | - \frac{\eta_0}{2};
\hat{x}(\mu) \cos \theta^*_l - y(\mu) | \sin \theta^*_l | + \frac{\eta_0}{2})
$.

Let us compute $\mu(T_l)$, where 
\[
  T_l = \bigl\lbrace
  \hat{y}(\mu) \in (y(\mu) - \frac{\eta_0}{2}, y(\mu) + \frac{\eta_0}{2}) \colon
  \hat{x}(\mu) \cos \theta^*_l - y(\mu) \sin \theta^*_l \in \Phi(E)
  \bigl\rbrace.
\]

Let us also define a function $L_l \colon \real \to \real$
by a formula:
$L_l(x) = \hat{x}(\mu) \cos \theta^*_l - x \sin \theta^*_l$.

We have:
\[
  \mu\big( (\hat{x}(\mu) \cos \theta^*_l - y(\mu) \sin \theta^*_l - \frac{\eta_0}{2} | \sin \theta^*_l|;
  \hat{x}(\mu) \cos \theta^*_l - y(\mu) \sin \theta^*_l + \frac{\eta_0}{2} | \sin \theta^*_l|)
  \cap \Phi(E) \geq
  \big)
\]
\[
  \eta_0 |\sin \theta^*_l| \big(1 - \varepsilon\frac{2 \eta_0}{\eta_0 |\sin \theta^*_l|}\big) =
\]
\[
  \eta_0 |\sin \theta^*_l| - 2 \varepsilon \eta_0.
\]
Next, we have
\[
\mu(T_l) \geq \frac{1}{|\sin \theta^*_l|} \big( \eta_0 |\sin \theta^*_l| - 2 \varepsilon \eta_0\big) =
\]
\[
\eta_0 \big(1 - \frac{2 \varepsilon}{|\sin \theta^*_l|}\big).
\]

Next,
\[
  \mu\big(y(\mu) - \frac{\eta_0}{2}; y(\mu) + \frac{\eta_0}{2} \setminus T_l\big) \leq
\]
\[
  \frac{2 \varepsilon \eta_0}{|\sin \theta^*_l|}.
\]
Hence
\[
  \mu\big( \bigcup_{l = 0}^{k - 1} \big(y(\mu) - \frac{\eta_0}{2};
  y(\mu) + \frac{\eta_0}{2} \setminus T_l\big) \leq
  \sum_{l = 0}^{k - 1} \frac{2 \varepsilon \eta_0}{|\sin \theta^*_l|}.
\]
Thus
\[
  \mu\big(\bigcap_{l=0}^{k-1} T_l \big) \geq \eta_0
  \big(1 - \sum_{l = 0}^{k - 1} \frac{2 \varepsilon}{|\sin \theta^*_l|}\big).
\]
and by inequality (\ref{equation:four-sinus}) we obtain
\[
  \mu\big(\bigcap_{l=0}^{k-1} T_l \big) \geq \frac{\eta_0}{2},
\]
hence $\bigcap_{l=0}^{k-1} T_l\not=\emptyset$.
Thus, let us choose any 
$\yxMu \in \bigcap_{l=0}^{k-1} T_l$
(We use this notation because $\hat{y}$ depends on $\hat{x}$).

Denote $X^*_0 = (\hat{x}(\mu), \yxMu)$.
We have that
\[
  \left\{
    \begin{array}{l}
    |\hat{x}(\mu) - x(\mu)| < \frac{\eta_0}{2} \\
    |\yxMu - y(\mu)| < \frac{\eta_0}{2},
    \end{array}
  \right.
\]
so in particular $\phi_{\mu}(X^*_0) \in W^*$.
Moreover, we have
\begin{equation}\label{equation:conclusion-3}
\hat{x}(\mu) \cos \theta^*_l - \yxMu \sin \theta^*_l \in \Phi(E).
\end{equation}
We will show that $\phi_\mu(X^*_0) \in \tilde{E}$.
Since (\ref{equation:conclusion-3}) we have
$\pi_x(\phi_{\theta^*_l}(X^*_0)) \in \Phi(E)$,
hence
$\pi_x(\phi_{\theta_l}^{-1}(\phi_{\mu}(X^*_0))) \in \Phi(E)$,
so
$\phi_{\mu}(X^*_0) \in \phi_{\theta_l}[\phi(E) \times \real]$.
Therefore
$\phi_{\mu}(X^*_0) \in \tilde{E}$.
\end{proof}

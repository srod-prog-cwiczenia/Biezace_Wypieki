...
\begin{theorem}
%%%%Assume that $\cK$ is a collection of subsets of $\real$ such that each element $\cK$ has an empty interior: $\forall_{K\in\cK} \int(K) = \emptyset$.
Suppose that $\tau_2$ is a topology on $\real$ stronger than $\tau$ 
such that 
$\forall_{U \in \tau_2\setminus\lbrace \emptyset \rbrace} 
\exists_{V\in\tau\setminus\lbrace\emptyset\rbrace}  
\mathint (U \bigtriangleup V) = \emptyset$.
...
\bibitem[GS]{GS}
  Z.Grande, E.Stro\'nska, {\em On an ideal of linear sets},

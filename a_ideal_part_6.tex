...
\begin{theorem}
Assume that $\cK$ is a collection of subsets of $\real$ such
that each element $\cK$ has an empty interior:
$\forall_{K\in\cK} \int(K) = \emtpyset$.
Suppose that $\tau_2$ is a topology on $\real$ stronger than $\tau$ 
such that 
$\forall_{U \in \tau_2\setminus\lbrace \emtpyset \rbrace} \exists_{V\in\tau} \exists_{K_1, K_2 \in \cK} U = V \setminus K_1 \cup K_2 $.

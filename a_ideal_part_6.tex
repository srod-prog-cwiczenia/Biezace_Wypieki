...
We answer the question from the article \cite{FG} by constructing
...
Notice that for the case $\tau_1 = \tau_e$
and $\tau_2 = \tau_d$ we obtain the
classical ideal $\aideal$.
...
$\aidealprime$ (recall that
$\aidealprime = \lbrace X \subseteq \real \colon
\exists_{E \subseteq \real} E\text{\ is\ closed\ and\ } X \subseteq E \setminus \Phi(E)\rbrace$
):
\begin{definition}
...
\section{Special countable set $\aideal$}
The aim of this section is to give a solution to
the following problem from \cite{FG} (see Open Question, page 6
from \cite{FG}): Are the ideals generated by $\aideal$ and
$\aidealprime$ equal?
...
Thus, in both cases we can find an open set $U_1$ such that
...
\end{proof}
\end{proof} %of the main construction/theorem
